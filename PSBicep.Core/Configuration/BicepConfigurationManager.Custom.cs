...
// Customizations and additions to our clone of Bicep.Core.Configuration.ConfigurationManager
    ...
    public BicepConfigInfo GetConfigurationInfo(BicepConfigScope mode, Uri sourceFileUri)
    {
        ...
        switch (mode)
        {
            ...
            case BicepConfigScope.Local:
                ...
                {
                    var fileHandle = fileExplorer.GetFile(config.ConfigFileUri.Value);
                    var configString = JsonElementFactory.CreateElementFromStream(fileHandle.OpenRead()).ToString();
                    return new BicepConfigInfo(config.ConfigFileUri?.Path, configString);
                }
